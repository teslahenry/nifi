...
@CapabilityDescription("Executes a HiveQL DDL/DML command (UPDATE, INSERT, e.g.). The property HiveQLScript of an incoming FlowFile is expected to be the HiveQL command "
...
public class ExecuteHive3QLScript extends AbstractHive3QLProcessor {
    ...
    public static final PropertyDescriptor HIVEQL_SCRIPT = new PropertyDescriptor.Builder()
            .name("hiveql-script")
            .displayName("HiveQL script")
            .description("The preferred hiveql commands to put to database to execute")
            .required(true)
            .addValidator(StandardValidators.NON_EMPTY_VALIDATOR)
            .defaultValue("${hive.ql.script}")
            .build();
    ...
    static {
        ...
        _propertyDescriptors.add(HIVEQL_SCRIPT);
...
//        process.fetchFlowFiles(fetchFlowFiles);
    ...
    private Put.PutFlowFile<FunctionContext, Connection> putFlowFile = (context, session, fc, conn, flowFile, result) -> {
        final String script = context.getProperty(HIVEQL_SCRIPT).evaluateAttributeExpressions(flowFile).getValue();
